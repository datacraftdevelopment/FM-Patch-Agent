...
if [ -x /usr/local/bin/FMDeveloperTool ] && [ -x "$PY" ]; then
  ...
  "$PY" "$ROOT/scripts/fm_export.py" "$SB/dev.fmp12"  -o "$SB/dev.xml"  --stamp-guids
  "$PY" "$ROOT/scripts/fm_export.py" "$SB/prod.fmp12" -o "$SB/prod.xml" --stamp-guids
  "$PY" "$ROOT/scripts/saxml_parser.py" "$SB/dev.xml"  -o "$SB/dev_parsed"
  "$PY" "$ROOT/scripts/saxml_parser.py" "$SB/prod.xml" -o "$SB/prod_parsed"
  "$PY" "$ROOT/scripts/saxml_diff.py" "$SB/dev_parsed" "$SB/prod_parsed" -o "$SB/diff.json"
